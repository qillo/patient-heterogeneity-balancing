...
\label{sec:soa_scope_and_metholodogy}

The aim of this chapter is to review the current literature carried out in the field of T1-Diabetes glycemic prediction. In order to build a solid review, particular attention is given to three separate -- while closely related -- areas: T1-Diabetes prediction using CGM data, data balancing techniques applied to generic machine learning problems and time-series forecasting, and data balancing approaches for T1D prediction.

To perform this review, several academic and scientific repositories were consulted, including Google Scholar~\cite{google_scholar}, PubMed~\cite{pubmed}, IEEE Xplore~\cite{ieee_xplore}, ScienceDirect~\cite{sciencedirect}, and SpringerLink~\cite{springerlink}. Searches combined terms related to T1D and glucose forecasting with terms such as \textit{data balancing}, \textit{imbalanced regression}, \textit{SMOTER}, and \textit{SMOGN}. The selection of literature was then filtered by its relevance to the scope of this study.

\section{Glycemic Prediction in T1D}
\label{sec:soa_glycemic_prediction}

\subsection{Evolution of Predictive Architectures}
\label{subsec:soa_prediction_evolution}

Glycemic prediction has recently evolved from simple statistical and physiological approaches towards machine learning. Broad reviews from 2019 \cite{WOLDAREGAY2019109} and 2022 \cite{info:doi/10.2196/34699} state how RNN, SVM, Gaussian and hybrid approaches currently co-exist, while also emphasizing the fact that, for now, there is not an universal model that performs excellently for all clinical contexts. Recent works \cite{WOLDAREGAY2019109, info:doi/10.2196/34699} follow-up with: LSTM, GRU-based, proposals incorporate attention mechanisms, Transformers and GNN to capture higher complexity. Furthermore, literature reviews \cite{info:doi/10.2196/34699} state the field has bifurcated into two families: continuous regression of future glucose values and event prediction (primarily hypoglycemia), formulated as a classification problem for early warnings.

\subsection{Prediction Horizons and History Length}
\label{subsec:soa_prediction_horizons_and_history}

The prediction horizon defines how far into the future the glucose level is predicted. The scientific literature \cite{WOLDAREGAY2019109, info:doi/10.2196/34699, s26092675, 10.1371/journal.pone.0253125} converges on the idea that short-medium horizons -- 30 min to 1 hour -- are the most realistic and clinically useful. While longer horizons provide more time for preventive action, they also increase the degree uncertainty introduced by other external future factors: meals, insulin doses, physical activity, or any other that may alter the glucose level. Additionally, prediction errors also increase as this horizon lengthens.

The history window determines how much previous CGM information is provided to the model. Literature also converges on the idea that most reusable evidence for decision support is concentrated in windows of at least up to one hour before \cite{WOLDAREGAY2019109, info:doi/10.2196/34699, s26092675, 10.1371/journal.pone.0253125}. Although no "universally optimal" length has been established, these minimum 1 hour size windoes are commonly used, since they help the model identify the current glucose level, trend, rate of change, and variability.

\subsection{Evaluation Metrics and Clinical Taxonomy}
\label{subsec:soa_clinical_metrics}

Most studies \cite{WOLDAREGAY2019109} in the field use the typical regression problems measures: RMSE, MAE, MSE, etc; since they provide the most direct and uncomplicated numerical estimation of the difference between the predicted and the real values, and allow comparing models in terms of "average numerical accuracy". However, for problems like this, average error metrics may not be sufficient: a model can obtain a low RMSE while still failing to detect rare but clinically dangerous events. For this reason, the evaluation of glycemic prediction requires both statistical and clinical metrics: these models' evaluation must also consider where the error occurs, not only how large the average error is. As described in \ref{TIR-explanation}, the standard consensus amongst all literature is "glycemic ranges", often referred as Time In Range \cite{10.2337/dci19-0028}. These metrics are specially relevant for this thesis because they clearly define separate ranges according to their actual clinical relevance, which will allow us to directly quantify the imbalance of the glucose distribution.
...
\begin{figure}[htbp]
    ...
    \caption{Clarke Error Grid, classifies glucose predictions according to their clinical risk. Source: \cite{ceg_graph}}
