...
\begin{figure}[htbp]
    ...
    \caption{Balanced distribution on semi-balanced target, methods comparison.}
...
\begin{figure}[htbp]
    ...
    \caption{Balanced distribution on fully-balanced target, methods comparison.}
...
This section presents a general overview of the results obtained from all the different 27 experimental configuration described previously. The purpose here is not to reproduce these results statistically -- as presented later in Sections~\ref{sec:diatrend_results}--
\ref{sec:t1diabetesgranadaresults}, but to identify the principal patterns that are common to the three datasets. The complete numerical results for each of the 27 experiments performed in this thesis can be found in Appendix~\ref{app:experimental_results}.

% ============================================================
% TARGET DISTRIBUTION VS BALANCING METHOD
% ============================================================

\subsection{Influence of Target Distribution and Balancing Method}
\label{subsec:general_target_method}

An initial reasonable question is whether predictive behaviour is primarily determined by the selected target distribution or by the oversampling algorithm. To answer this question, the figures below present a visual comparison between the maximum variation observed across the different target distributions and the maximum variation observed across the different resampling methods, for each of the glycemic ranges.

% ------------------------------------------------------------
% DIATREND TARGET VS METHOD
% ------------------------------------------------------------

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{general_results/DiaTrend_method_vs_target_effect.png}
    \caption{Comparison between the variation associated with the target
    distribution and the maximum variation across balancing methods in
    DiaTrend.}
    \label{fig:diatrend_method_vs_target}
\end{figure}


% ------------------------------------------------------------
% REPLACE-BG TARGET VS METHOD
% ------------------------------------------------------------

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{general_results/REPLACE-BG_method_vs_target_effect.png}
    \caption{Comparison between the variation associated with the target
    distribution and the maximum variation across balancing methods in
    REPLACE-BG.}
    \label{fig:replacebg_method_vs_target}
\end{figure}


% ------------------------------------------------------------
% T1DIABETESGRANADA TARGET VS METHOD
% ------------------------------------------------------------

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{general_results/T1DiabetesGranada_method_vs_target_effect.png}
    \caption{Comparison between the variation associated with the target
    distribution and the maximum variation across balancing methods in
    T1DiabetesGranada.}
    \label{fig:t1diabetesgranada_method_vs_target}
\end{figure}


Figures~\ref{fig:diatrend_method_vs_target}--\ref{fig:t1diabetesgranada_method_vs_target} show a common pattern between the three datasets: the differences associated to the target distribution are considerably larger than the differences associated to the particular
oversampling method. This is also common across the different glycemic ranges, and especially evident in the hypoglycemic ranges. Therefore, the most influential experimental factor seems to be the \textit{balancing target}, rather than the mechanism used to produce the additional samples. However, this result does not imply that the three balancing methods are statistically equivalent, since formal equivalence between them was not tested.

% ============================================================
% GENERAL PREDICTIVE PERFORMANCE
% ============================================================

\subsection{Overall Effect of the Target Distribution}
\label{subsec:general_target_effect}

Figures~\ref{fig:diatrend_general_results}--\ref{fig:t1diabetesgranada_general_results} summarize the two most critical dimensions used in this thesis: clinical acceptability (A+B), and numerical prediction accuracy (RMSE). The results of the different resampling methods are represented by their mean. This aggregation is used only as a descriptive summary, and error bars representing standard deviation across the three methods are also presented.

% ------------------------------------------------------------
% DIATREND GENERAL RESULTS
% ------------------------------------------------------------

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{general_results/DiaTrend_general_results.png}
    \caption{General prediction performance for DiaTrend across the three
    target distributions. Bars represent the mean across random resampling,
    SMOTER, and SMOGN, while error bars represent $\pm 1$ descriptive
    standard deviation across methods.}
    \label{fig:diatrend_general_results}
\end{figure}


% ------------------------------------------------------------
% REPLACE-BG GENERAL RESULTS
% ------------------------------------------------------------

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{general_results/REPLACE-BG_general_results.png}
    \caption{General prediction performance for REPLACE-BG across the three
    target distributions. Bars represent the mean across random resampling,
    SMOTER, and SMOGN, while error bars represent $\pm 1$ descriptive
    standard deviation across methods.}
    \label{fig:replacebg_general_results}
\end{figure}


% ------------------------------------------------------------
% T1DIABETESGRANADA GENERAL RESULTS
% ------------------------------------------------------------

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{general_results/T1DiabetesGranada_general_results.png}
    \caption{General prediction performance for T1DiabetesGranada across
    the three target distributions. Bars represent the mean across random
    resampling, SMOTER, and SMOGN, while error bars represent $\pm 1$
    descriptive standard deviation across methods.}
    \label{fig:t1diabetesgranada_general_results}
\end{figure}


Overall, the three datasets show common patterns. The clearest effect always appears in the hypoglycemic ranges. Under the normalized baseline, both TBR$_2$ and TBR$_1$ showed very low clinical acceptability across the three datasets, despite the acceptable global performance. Increasing balancing intensity improved both ranges, with full balancing producing the strongest results and substantial RMSE reductions. This improvement comes with some trade-offs in the numerical accuracy of the model. Full balancing increases global RMSE, mainly due to reduced precision in TIR. However, both TIR and global clinical acceptability (A+B) increase under full balancing, showing that a higher numerical error does not necessarily imply worse clinical performance. The hyperglycemic ranges follow a similar although more moderate pattern. TAR$_2$ generally improves with stronger balancing, while TAR$_1$ shows only a small localized deterioration under full balancing. In summary, these summarized results show that the strongest balancing target produces very considerable improvements in the hypoglycemic ranges, while introducing only comparatively small limitations in the rest of ranges and evaluation metrics.
...
As defined in Section~\ref{sec:statistical_analysis}, the statistical analysis evaluates whether full balancing improves the proportion of clinically acceptable predictions both in hypoglycemia and severe hypoglycemia -- denoted as \(AB_{\mathrm{TBR_1}}\) and \(AB_{\mathrm{TBR_2}}\) -- relative to the corresponding \texttt{NORMALIZED\_BASELINE} configuration.

For each hypoglycemic range, the difference between the fully balanced and normalized configurations was first calculated independently for each of the methods and cross-validation folds. Then, the three method-specific differences were averaged within each fold according to Equation~\ref{eq:fold_mean_delta_ab_hypo}.

\paragraph{TBR$_2$ analysis.}

Table~\ref{tab:diatrend_fold_tbr2_effect} presents the fold-level results for severe hypoglycemia.

\begin{table}[htbp]
    \centering
    \caption{DiaTrend fold-level prediction performance for TBR$_2$.}
    \label{tab:diatrend_fold_tbr2_effect}
    \begin{tabular}{cccc}
        \toprule
        \textbf{Fold}
...
Full balancing produced a positive improvement in every cross-validation fold, with values that range from \(47.54\) to \(79.62\) percentage points. Moreover, all 15 individual method--fold comparisons were positive, which suggests that this effect is also consistent across balancing methods.

The five observations used in the statistical test are therefore:
...
\begin{equation}
p_{\mathrm{TBR_2}} < \alpha
\qquad
(0.00016 < 0.01),
\end{equation}

the null hypothesis defined in Equation~\ref{eq:null_hypothesis_hypo} can be rejected for TBR$_2$ in DiaTrend, with a considerable degree of confidence.
...
\paragraph{TBR$_1$ analysis.}

Table~\ref{tab:diatrend_fold_tbr1_effect} presents the corresponding fold-level results for TBR$_1$.

\begin{table}[htbp]
\centering
\caption{DiaTrend fold-level prediction performance for TBR$_1$.}
\label{tab:diatrend_fold_tbr1_effect}
\begin{tabular}{cccc}
\toprule
\textbf{Fold}
& \textbf{Normalized A+B}
...
& $\boldsymbol{\Delta}$ \textbf{A+B} \\
\midrule
1 & 0.71\% & 54.53\% & +53.81 pp \\
2 & 1.53\% & 66.77\% & +65.24 pp \\
3 & 1.96\% & 68.60\% & +66.64 pp \\
4 & 1.39\% & 73.88\% & +72.48 pp \\
5 & 1.89\% & 76.60\% & +74.71 pp \\
\midrule
\textbf{Mean}
& \textbf{1.50\%}
& \textbf{68.07\%}
...
Full balancing produced a positive improvement in every cross-validation fold, with values ranging from 53.81 to 74.71 percentage points. Moreover, all 15 individual method--fold comparisons were positive, which suggests that this effect is also consistent across balancing methods.
...
Applying the same one-sided paired Student's $t$-test produced:

\begin{equation}
t_{\mathrm{TBR_1}}
=
\frac{66.58}{8.15/\sqrt{5}}
=
18.26.
\end{equation}
...
\begin{equation}
p_{\mathrm{TBR_1}} < \alpha
\qquad
(2.64 \times 10^{-5} < 0.01),
\end{equation}

the null hypothesis defined in Equation~\ref{eq:null_hypothesis_hypo} can be rejected for TBR$_1$ in DiaTrend, with a considerable degree of confidence.

Additionally, this statistical significance is accompanied by a substantial effect magnitude. Across the five folds, full balancing increased $AB_{\mathrm{TBR_1}}$ by an average of 66.58 percentage points: the mean value increased from 1.50\% under the normalized baseline to 68.07\% under full balancing.
...
Therefore, the primary confirmatory statistical results support the study hypothesis for DiaTrend: \textbf{increasing the representation of rare glycemic ranges through full balancing produces statistically significant and substantial improvements in the proportion of clinically acceptable predictions in both severe hypoglycemia (TBR$_2$) and hypoglycemia (TBR$_1$)}. The remaining secondary outcomes must subsequently determine whether these benefits are accompanied by acceptable changes in numerical performance, global performance, and the other glycemic ranges.

\subsection{Secondary Outcomes}
\label{subsec:diatrend_secondary_outcomes}

Together with the primary statistical analysis, a secondary analyses was performed to study the impact of balancing in the rest of regression and clinical metrics. The confirmatory statistical analyses, although focused on the two hypoglycemic ranges, should not be considered enough to determine the full effect.

\begin{quote}
\small
\textit{The secondary analysis uses the aggregate results obtained after combining the predictions from all folds for each experimental configuration. Consequently, the aggregate percentages reported in this subsection do not necessarily coincide exactly with the unweighted mean of the five fold-level percentages used in the previous statistical tests. In any case, full results can be checked in Appendix~\ref{app:experimental_results}. Considering the large number of experiments and the corresponding performance metrics, for the scope if this secondary analysis, results are presented \textbf{mostly summarized} through their mean and descriptive standard deviation. Also, the normalized configuration remains the reference for interpreting the changes introduced by semi and full-balancing.}
\end{quote}
...
Table~\ref{tab:diatrend_hypoglycemia} summarizes some critical metrics for TBR for each of the balancing targets. Normalized target is presented as a reference. \textbf{Zones B and C contained no predictions in these ranges, so A+B is equivalent to Zone A.}

Semi-balancing resulted in a substantial intermediate improvement, relative to the normalized baseline: TBR$_2$ RMSE decreased by \(13.13\) mg/dL and A+B increased by \(31.99\) percentage points; In TBR$_1$, RMSE decreased by \(9.41\) mg/dL and and A+B increased by \(27.39\) percentage points. These clinical gains were mainly produced by a redistribution from Zone D into Zone A.

Full balancing intensified this effect: TBR$_2$ RMSE decreased by \(23.40\) mg/dL and A+B increased by \(68.93\) percentage points. In TBR$_1$, RMSE decreased by \(17.07\) mg/dL and and A+B increased by \(68.75\) percentage points. The redistribution from Zone D into Zone A was also similarly pronounced.

Semi-balancing therefore provided a valid intermediate response, and full balancing produced a substantially larger improvement in both numerical accuracy and clinical acceptability. While not formally proven with a corresponding statistical test, these results suggest that the predictive performance improves progressively as the degree of balancing increases -- at least up to a fully symmetrical distribution.
...
The largest numerical deterioration of full balancing appeared in TIR, where RMSE was increased by \(9.10\) mg/dL and MAE by \(6.85\) mg/dL. Nevertheless, TIR A+B decreased by only \(0.49\) percentage points and remained at \(99.01\%\). Moreover, no TIR predictions entered Zones D or E.
...
The confirmatory statistical analyses rejected the null hypothesis for both hypoglycemic ranges in DiaTrend. Using the fold-level analysis, full balancing increased \(AB_{\mathrm{TBR_2}}\) by an average of \(63.75\) percentage points relative to the normalized baseline, with a one-sided \(p\)-value of \(0.00016\). For TBR$_1$, the corresponding mean improvement was \(66.58\) percentage points, with a one-sided \(p\)-value of \(2.64 \times 10^{-5}\).

The aggregate secondary results support these conclusions. When predictions from all folds were combined, mean TBR$_2$ A+B across methods increased from \(3.74\%\) under normalized balancing to \(72.68\%\) under full balancing. TBR$_1$ A+B similarly increased from \(1.60\%\) to \(70.35\%\). These improvements were accompanied by substantial reductions in RMSE, MAE, and Zone D.
...
The same statistical analysis defined in Section~\ref{sec:statistical_analysis} was also performed for REPLACE-BG.

\paragraph{TBR$_2$ analysis.}

Table~\ref{tab:replacebg_fold_tbr2_effect} presents the fold-level results for severe hypoglycemia.

\begin{table}[htbp]
    \centering
    \caption{REPLACE-BG fold-level prediction performance for TBR$_2$.}
    \label{tab:replacebg_fold_tbr2_effect}
    \begin{tabular}{cccc}
        \toprule
        \textbf{Fold}
        & \textbf{Normalized A+B}
...
The five observations used in the statistical are therefore:
...
the null hypothesis defined in Equation~\ref{eq:null_hypothesis_hypo} can be rejected for REPLACE-BG.
...
Table~\ref{tab:replacebg_fold_tbr1_effect} presents the corresponding fold-level results for TBR$_1$.

\begin{table}[htbp]
\centering
\caption{REPLACE-BG fold-level prediction performance for TBR$_1$.}
\label{tab:replacebg_fold_tbr1_effect}
\begin{tabular}{cccc}
\toprule
\textbf{Fold}
& \textbf{Normalized A+B}
...
& $\boldsymbol{\Delta}$ \textbf{A+B} \\
\midrule
1 & 0.48\% & 80.27\% & +79.79 pp \\
2 & 0.01\% & 78.90\% & +78.89 pp \\
3 & 0.68\% & 77.63\% & +76.94 pp \\
4 & 0.08\% & 78.16\% & +78.07 pp \\
5 & 0.38\% & 77.39\% & +77.01 pp \\
\midrule
\textbf{Mean}
& \textbf{0.33\%}
& \textbf{78.47\%}
...
Full balancing produced a positive improvement in every cross-validation fold, with improvements ranging from 76.94 to 79.79 percentage points. Moreover, all 15 individual method--fold comparisons were positive, which suggests that this effect is also consistent across balancing methods.
...
the null hypothesis defined in Equation~\ref{eq:null_hypothesis_hypo} can be rejected for TBR$_1$, with a considerable degree of confidence.

Additionally, this statistical significance is accompanied by a substantial effect magnitude. Across the five folds, full balancing increased $AB_{\mathrm{TBR_1}}$ by an average of 78.14 percentage points: the mean value increased from 0.33\% under the normalized distribution to 78.47\% under full balancing.

\paragraph{Interpretation}

One more, for REPLACE-BG, the improvement was consistent across the complete experimental configuration. Every fold showed a positive mean difference, and every individual comparison involving random resampling, SMOTER, or SMOGN favoured full balancing. Consequently, the result does not appear to depend on a particular balancing method or cross-validation partition.

Therefore, the primary confirmatory statistical results support the study hypothesis also for REPLACE-BG. The remaining secondary outcomes must subsequently determine whether these benefits are accompanied by acceptable changes in numerical performance, global performance, and the other glycemic ranges.
...
The confirmatory statistical analyses rejected the null hypothesis for both hypoglycemic ranges in REPLACE-BG. Using the fold-level analysis, full balancing increased \(AB_{\mathrm{TBR_2}}\) by an average of \(76.81\) percentage points relative to the normalized baseline, with a one-sided \(p\)-value of \(8.00 \times 10^{-8}\). For TBR$_1$, the corresponding mean improvement was \(78.14\) percentage points, with a one-sided \(p\)-value of \(7.24 \times 10^{-9}\).

The aggregate secondary results support these conclusions. When predictions from all folds were combined, mean TBR$_2$ A+B across methods increased from \(0.01\%\) under normalized balancing to \(76.89\%\) under full balancing. TBR$_1$ A+B similarly increased from \(0.33\%\) to \(78.49\%\). These improvements were accompanied by substantial reductions in RMSE, MAE, and Zone D.

As in Diatrend, the balancing target was found to have a much greater influence on predictive behaviour than the selected resampling method. Random resampling, SMOTER, and SMOGN produced closely aligned aggregate results under the same target distribution, although formal equivalence between them was not tested either.

Semi-balancing provided a favourable intermediate response. It substantially improved both hypoglycemic ranges while preserving global RMSE and producing only a limited deterioration in TIR numerical accuracy.

Overall, full balancing is considered the most beneficial REPLACE-BG configuration. The moderate deterioration in global and TIR numerical accuracy is definitely outweighed by the magnitude and clinical relevance of the improvements obtained in TBR.
...
Table~\ref{tab:t1diabetesgranada_fold_tbr2_effect} presents the fold-level results for severe hypoglycemia.

\begin{table}[htbp]
    \centering
    \caption{T1DiabetesGranada fold-level prediction performance for TBR$_2$.}
    \label{tab:t1diabetesgranada_fold_tbr2_effect}
    \begin{tabular}{cccc}
        \toprule
        \textbf{Fold}
        & \textbf{Normalized A+B}
...
the null hypothesis defined in Equation~\ref{eq:null_hypothesis_hypo} can definitely be rejected for T1DiabetesGranada.

\paragraph{TBR$_1$ analysis.}

Table~\ref{tab:t1diabetesgranada_fold_tbr1_effect} presents the corresponding fold-level results for TBR$_1$.

\begin{table}[htbp]
\centering
\caption{T1DiabetesGranada fold-level prediction performance for TBR$_1$.}
\label{tab:t1diabetesgranada_fold_tbr1_effect}
\begin{tabular}{cccc}
\toprule
\textbf{Fold}
& \textbf{Normalized A+B}
...
& $\boldsymbol{\Delta}$ \textbf{A+B} \\
\midrule
1 & 8.02\% & 79.32\% & +71.30 pp \\
2 & 7.59\% & 77.35\% & +69.76 pp \\
3 & 5.65\% & 81.30\% & +75.65 pp \\
4 & 10.66\% & 78.45\% & +67.79 pp \\
5 & 10.17\% & 79.05\% & +68.88 pp \\
\midrule
\textbf{Mean}
& \textbf{8.42\%}
& \textbf{79.09\%}
...
Full balancing produced a positive improvement in every cross-validation fold, with values ranging from 67.79 to 75.65 percentage points. Moreover, all five fold-level differences were clearly positive, indicating a highly consistent improvement across the cross-validation partitions.
...
the null hypothesis defined in Equation~\ref{eq:null_hypothesis_hypo} can be rejected for TBR$_1$ in T1DiabetesGranada, with a considerable degree of confidence.

Additionally, this statistical significance is accompanied by a substantial effect magnitude. Across the five folds, full balancing increased $AB_{\mathrm{TBR_1}}$ by an average of 70.68 percentage points: the mean value increased from 8.42\% under the normalized baseline to 79.09\% under full balancing.

\paragraph{Interpretation}

One again, for T1DiabetesGranada, and as in Diatrend and REPLACE-BG, the improvement was consistent across the complete experimental configuration. Every fold showed a positive mean difference, and every individual comparison involving random resampling, SMOTER, or SMOGN favoured full balancing. Consequently, the result does not appear to depend on a particular balancing method or cross-validation partition.

Therefore, the primary confirmatory statistical results support the study hypothesis also for T1DiabetesGranada. The remaining secondary outcomes must subsequently determine whether these benefits are accompanied by acceptable changes in numerical performance, global performance, and the other glycemic ranges.

\subsection{Secondary Outcomes}
\label{subsec:t1diabetesgranadasecondary_outcomes}

The secondary outcomes analysis follows the same procedure used for DiaTrend and REPLACE-BG datasets.
...
The primary statistical analysis rejected the null hypothesis for T1DiabetesGranada. Using the fold-level analysis, full balancing increased \(AB_{\mathrm{TBR_2}}\) by an average of \(75.93\) percentage points relative to the normalized baseline, with a one-sided \(p\)-value of \(3.40 \times 10^{-6}\). For TBR$_1$, the corresponding mean improvement was \(70.68\) percentage points, with a one-sided \(p\)-value of \(4.22 \times 10^{-7}\).
...
Table~\ref{tab:cross_dataset_primary_results} summarizes the primary statistical results across the three datasets and both hypoglycemic ranges.

\begin{table}[htbp]
\centering
\caption{Cross-dataset comparison of the primary statistical outcomes.}
...
Full balancing produced a statistically significant improvement in both hypoglycemic ranges across all three datasets. The effects obtained for REPLACE-BG and T1DiabetesGranada were generally larger and less variable, whereas DiaTrend showed slightly smaller, although still substantial, improvements.

\subsection{Holm-Adjusted Statistical Results}
\label{subsec:holm_adjusted_results}

Since the confirmatory hypothesis was evaluated for both hypoglycemic ranges across three datasets, the corresponding $p$-values were adjusted using the Holm procedure \cite{enwiki:1352765388}.

\begin{table}[htbp]
\centering
\caption{Original and Holm-adjusted $p$-values for the primary statistical outcomes.}
...
All adjusted $p$-values remained below the predefined significance level of $\alpha = 0.01$. Therefore, the correction did not alter any of the dataset- or range-specific conclusions.

\subsection{Consistency of the Primary Outcomes}
\label{subsec:primary_outcome_consistency}

The effects were therefore consistent across the three datasets and both hypoglycemic ranges: full balancing improved $AB_{\mathrm{TBR_2}}$ and $AB_{\mathrm{TBR_1}}$ in every fold and in all individual method--fold comparisons. Apart from the slightly smaller improvements observed in DiaTrend, the results were analogous and consistently supported the study hypothesis.
