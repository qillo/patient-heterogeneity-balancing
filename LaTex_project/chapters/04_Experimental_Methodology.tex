...
\subsection{Proposed Hybrid Balancing Strategy}
...
\label{target_distributions_chapter}
...
These values represent the theoretical redistribution of the complete datasets. There is some small residual differences caused by conversion to integer. These rounding differences will also be addressed in the implemenation. By applying this re-distribution all the three datasets will share common proportions amongst the glycemic ranges, regardless of their total size.
...
Thus, if (N) denotes the total number of training windows, each range is resampled to contain approximately (N/5) samples. Ranges below this target are oversampled, whereas ranges above it are undersampled.

\textbf{\textit{This approach is considered the main strategy against which the study premise will be evaluated.}}
...
In order to isolate the effect of the previously proposed balancing strategies, several experimental conditions will remain fixed throughout all experiments in order to provide a stable baseline for comparison. These decisions have been chosen because they represent common standard settings in glucose prediction studies and because the objective of this thesis is not to compare different predictive architectures, prediction horizons, or window configurations, but to evaluate how modifying the training-data distribution affects glucose prediction performance under equivalent modelling conditions.
...
\label{subsec:experimental_methodology_workflow}

The experimental procedure is structured as a sequence of stages designed to isolate the effects of each methodological decision on glucose prediction performance. The workflow evaluates three balancing targets and the three balancing methods, while implementing patient-aware mechanisms and keeping the validation and test sets unchanged in all cases.

\textbf{First}, for each dataset, three training-set versions will be generated according to the normalized baseline target distribution, using each of the three proposed methods: random, SMOTER and SMOGN. The same fixed training configuration described in Section~\ref{subsec:experimental_methodology_fixed_configuration} will be used across all balanced training-set versions. Comparing the three methods under the same target distribution makes it possible to determine whether regression-aware synthetic oversampling provides benefits beyond basic random resampling. 

\textbf{Second}, following the same procedure used for the normalized baseline, the three same balancing methods will be applied using the semi-balanced target distribution, in order to evaluate the effects of this intermediate balancing strategy.

\textbf{Third}, analogously to the previous steps, the same methods be applied using the fully balanced target distribution, in which all five glycemic ranges receive equal representation. This stage evaluates the effects of the most intensive balancing configuration and determines whether its potential improvements in clinically relevant minority ranges compensate for any degradation in overall predictive performance.

\textbf{Finally}, each resulting model will be evaluated using the same unmodified validation and test data. Performance will then be analysed using both numerical and clinical metrics, with particular to the minority regions identified during the EDA.
...
The general hypothesis states that \textbf{increasing the representation of rare glycemic ranges during training has a positive effect on predictive performance in clinically critical regions}. However, this formulation is too broad, since it englobes the full cohort of the problem. For the scope of this analysis, and in order to perform the evaluation through a single statistical test, the hypothesis is simplified to one primary clinical outcome. More specifically, the statistical analysis focuses on severe hypoglycemia, represented by TBR$_2$, because it is the most clinically critical range as stated in the previous chapters. 

\subsection{Primary Clinical Outcome}
\label{subsec:primary_clinical_outcome}

Formally, the selected outcome for the statistical analysis is the global proportion of severe-hypoglycemia predictions located in Clarke Error Grid Zones A and B, denoted as:

\begin{equation}
AB_{\mathrm{TBR_2}}.
\end{equation}

Zone A contains predictions considered clinically accurate, whereas Zone B contains predictions that deviate from the reference glucose value but would not normally lead to an inappropriate clinical decision. Therefore, combined, they represent the percentage of predictions that remain \textbf{clinically acceptable}.

\subsection{Primary Effect Measure}
\label{subsec:primary_effect_measure}

The reference baseline for all datasets is defined by the results of the corresponding \texttt{NORMALIZED BASELINE} experiment, since it uses a common targets distribution for the three of them. Then, the fully balanced configuration is compared with the corresponding normalized configuration.

For each balancing method \(m\) and cross-validation fold \(f\), the effect
is defined as:

\begin{equation}
\Delta AB_{\mathrm{TBR_2}}^{(m,f)}
=
AB_{\mathrm{TBR_2}}^{(m,f,\mathrm{full})}
-
AB_{\mathrm{TBR_2}}^{(m,f,\mathrm{normalized})},
\label{eq:delta_ab_tbr2}
\end{equation}
...
A positive value of \(\Delta AB_{\mathrm{TBR_2}}^{(m,f)}\) indicates that full balancing increases the proportion of clinically acceptable severe-hypoglycemia predictions relative to the normalized baseline for the corresponding method and fold. A negative value indicates the opposite.
...
\begin{equation}
\Delta AB_{\mathrm{TBR_2}}^{(f)}
=
\frac{1}{3}
\sum_{m=1}^{3}
\Delta AB_{\mathrm{TBR_2}}^{(m,f)}.
\label{eq:fold_mean_delta_ab_tbr2}
\end{equation}
...
Let \(\mu_{\Delta}\) denote the mean paired difference in \(AB_{\mathrm{TBR_2}}\) between the fully balanced and normalized configurations across the five cross-validation folds, after averaging the corresponding differences across the three balancing methods.
...
H_0:
\mu_{\Delta} \leq 0.
\label{eq:null_hypothesis_tbr2}
\end{equation}

The alternative hypothesis states that full balancing produces a positive mean improvement in severe-hypoglycemia predictions clinical acceptability:

\begin{equation}
H_1:
\mu_{\Delta} > 0.
\label{eq:alternative_hypothesis_tbr2}
\end{equation}

Rejecting ($H_0$) with a ($p$)-value below the selected significance level would provide enough evidence for our research premise: that full balancing improves \(AB_{\mathrm{TBR_2}}\) relative to the normalized baseline across all the evaluated balancing methods. If proven, this statistical evidence will then have to be evaluated together with any possible deterioration in other clincal metrics, and with the actual magnitude of \(\mu_{\Delta}\). This is to determine whether the observed change is practically meaningful -- \textit{satisfying Equation~\ref{eq:alternative_hypothesis_tbr2} does not necessarily imply that the observed improvement is clinically relevant enough.}

\subsection{Statistical Test}
\label{subsec:statistical_test}

The primary hypothesis is evaluated using a one-sided paired Student's \(t\)-test. The paired design is appropriate because every fully balanced result is compared with the normalized result obtained using the same dataset and experimental configuration.

Let $d_f$ denote the paired difference for each of the five folds as defined in Equation~\ref{eq:fold_mean_delta_ab_tbr2}. The mean paired difference is then calculated as:
...
The paired \(t\)-test assumes that the fold-level paired differences are approximately normally distributed. However, in this case, this assumption cannot be formally assesed, since only five fold-level differences are available for each dataset.

Additionally, since the same primary hypothesis is tested independently for DiaTrend, REPLACE-BG, and T1DiabetesGranada, the resulting \(p\)-values will also be studied using the Holm procedure \cite{enwiki:1352765388} in order to also assess the probability of obtaining false-positive conclusions across the three dataset-specific tests.

\subsection{Secondary Outcomes}
\label{subsec:secondary_statistical_outcomes}

The primary confirmatory analysis focuses exclusively on the difference in \(AB_{\mathrm{TBR_2}}\) between full balancing and the normalized baseline. However, this outcome alone cannot describe the complete effect of modifying the training-data distribution.
...
These secondary outcomes will be used to determine the impact of the primary clinical improvement -- if proven -- on other numerical prediction error metrics and on the movement of observations from clinically dangerous into acceptable ones. They are not treated as additional confirmatory hypotheses. They will mainly be interpreted through their numerical changes relative to the normalized baseline, and their consistency across balancing methods and datasets.

\section{Balancing Implementation}
\label{sec:balancing_implementation}
...
The complete experimental execution are run on the Calculus computing server. Training runs are submitted as batch jobs through \textbf{SLURM}.

This server-based workflow is necessary because of the large number of glucose windows and the computational cost of training all 27 -- 3 datasets x 3 methods x 3 targets -- balancing configurations across the different folds.